...
public class Problem055LychrelNumbers {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 1; i < 10_000; i++) {
      ...
      for (int j = 1; j <= 50 && !isPalindrome; j++) {
        bi = addReversed(bi);
        isPalindrome = EulerUtils.isPalindrome(bi.toString());
        if (isPalindrome) {
      ...
      if (!isPalindrome) {
        lychrelCount++;
        System.out.println(i + " : " + bi + " : does not produce a palindrome");
      }
    }
    System.out.println(lychrelCount);
  }

  public static BigInteger addReversed(BigInteger i) {
    BigInteger reversed = new BigInteger(EulerUtils.reverse(i + ""), 10);
    return i.add(reversed);
  }
